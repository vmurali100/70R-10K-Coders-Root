...
export const getAllUsers = createAsyncThunk("users/getAllUsers", () => {
  return fetch("http://localhost:4011/user")
    .then((res) => res.json())
...
export const updateUserAction=createAsyncThunk('users/updateUser',(user,{dispatch})=>{
    return fetch("http://localhost:4011/user/"+user.id,{
...
return fetch("http://localhost:4011/user/"+user.id,{method:"DELETE"}).then((res)=>{
