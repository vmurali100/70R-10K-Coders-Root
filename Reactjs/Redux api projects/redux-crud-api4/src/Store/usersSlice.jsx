...
export const getAllUsers = createAsyncThunk('users/getAllUsers', () => {
    return fetch("http://localhost:3002/students").then((res) => res.json()).catch((err) => console.log(err))
}

)
export const createUserAction =createAsyncThunk('users/createUser',(user,{dispatch})=>{
    return fetch("http://localhost:3002/students/",{method:'POST',body:JSON.stringify(user), headers: {
...
export const updateUserAction = createAsyncThunk('users/updateUser',(user,{dispatch})=>{
    return fetch("http://localhost:3002/students/"+user.id,{method:'PUT',body:JSON.stringify(user), headers: {
        'Accept': 'application/json, text/plain, */*',
        'Content-type':'application/json'
    },}).then(()=>{
        dispatch(getAllUsers())
...
export const deleteUserAction = createAsyncThunk('users/deleteUser', (user,{dispatch}) => {
    return fetch("http://localhost:3002/students/" + user.id, { method: 'DELETE' }).then((res) =>{
