
import { Prateep1 } from "./Prateep1";
import { Dharahaas1 } from "./Dharahaas1";
import { Dhruvaram1 } from "./Dhruvaram1";
import { Triveni1 } from "./Triveni1";
import { VimalaClass1 } from "./VimalaClassComp1";
import { PrateepClass1 } from "./PrateepClassComp1";
import { DharahaasClass1 } from "./DharahaasClassComp1";
import { DhruvaramClass1 } from "./DhruvaramClassComp1";
import { TriveniClass1 } from "./TriveniClassComp1";


export function Vimala1(){
    return <div>
        <h2>Hello from vimala1 component  from project2 </h2>
       <Prateep1/>
       <Dharahaas1/>
       <Dhruvaram1/>
<Triveni1/>
<VimalaClass1/>
<PrateepClass1/>
<DharahaasClass1/>
<DhruvaramClass1/>
<TriveniClass1/>
</div>
}