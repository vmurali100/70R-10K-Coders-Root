...
export const getAllComments = createAsyncThunk('comments/getAllComments', () => { //it brings data from api need to pass slice name and url to get data.
    return fetch("http://localhost:5003/comments/").then((res) => res.json()).catch((err) => console.log(err))
})

export const createCommentAction = createAsyncThunk('comments/createComments', (comment, { dispatch }) => {
    return fetch("http://localhost:5003/comments/", {
        method: 'POST',
...
export const deleteCommentAction = createAsyncThunk('comments/deleteComments', (comment, { dispatch }) => {
    return fetch("http://localhost:5003/comments/" + comment.id, {
        method: 'DELETE',
        body: JSON.stringify(comment),
        headers: {
            'Accept': 'application/json, text/plain, */*',
