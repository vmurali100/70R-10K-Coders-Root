...
export const getAllUsers = createAsyncThunk('users/getAllUsers',()=>{ // function is responsible to bring the data from server and will return it
    return fetch("http://localhost:4001/students/").then((res)=>res.json()).catch((err)=>console.log(err)) // it means after getting data from server we are giving call back funtn 
})
